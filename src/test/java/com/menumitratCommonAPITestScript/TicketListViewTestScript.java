...
public class TicketListViewTestScript extends APIBase
{
    ...
    @DataProvider(name = "getTicketListViewNegativeData")
    public Object[][] getTicketListViewNegativeData() throws customException {
        try {
            LogUtils.info("Reading ticket list view negative test scenario data");
            ExtentReport.getTest().log(Status.INFO, "Reading ticket list view negative test scenario data");
            ...
            if (filteredData.isEmpty()) {
                String errorMsg = "No valid ticket list view negative test data found after filtering";
                LogUtils.failure(logger, errorMsg);
                ExtentReport.getTest().log(Status.FAIL, MarkupHelper.createLabel(errorMsg, ExtentColor.RED));
                throw new customException(errorMsg);
            }
            ...
            LogUtils.failure(logger, "Error in getting ticket list view negative test data: " + e.getMessage());
            ExtentReport.getTest().log(Status.FAIL, "Error in getting ticket list view negative test data: " + e.getMessage());
            throw new customException("Error in getting ticket list view negative test data: " + e.getMessage());
        }
    }

    @Test(dataProvider = "getTicketListViewNegativeData")
    public void ticketListViewNegativeTest(String apiName, String testCaseid, String testType, String description,
            String httpsmethod, String requestBody, String expectedResponseBody, String statusCode) throws customException {
        try {
            LogUtils.info("Starting ticket list view negative test case: " + testCaseid);
            ExtentReport.createTest("Ticket List View Negative Test - " + testCaseid + ": " + description);
            ExtentReport.getTest().log(Status.INFO, "Test Description: " + description);
            
            if (apiName.equalsIgnoreCase("ticketlistview") && testType.equalsIgnoreCase("negative")) {
                requestBodyJson = new JSONObject(requestBody);
                
                LogUtils.info("Request Body: " + requestBodyJson.toString());
                ExtentReport.getTest().log(Status.INFO, "Request Body: " + requestBodyJson.toString());
                
                // Set payload for ticket list view request
                if (requestBodyJson.has("outlet_id")) {
                    ticketListViewRequest.setOutlet_id(requestBodyJson.getString("outlet_id"));
                }
            ...
            String errorMsg = "Error in ticket list view negative test: " + e.getMessage();
            LogUtils.exception(logger, errorMsg, e);
            ExtentReport.getTest().log(Status.FAIL, MarkupHelper.createLabel(errorMsg, ExtentColor.RED));
            if (response != null) {
                ExtentReport.getTest().log(Status.FAIL, "Failed Response Status Code: " + response.getStatusCode());
