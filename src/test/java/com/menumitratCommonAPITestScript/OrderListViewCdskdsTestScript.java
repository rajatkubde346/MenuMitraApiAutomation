...
public class OrderListViewCdskdsTestScript extends APIBase
{
    private JSONObject requestBodyJson;
    private Response response;
    private String baseURI;
    private String accessToken;
    private OrderRequest orderListViewCdskdsRequest;
    private URL url;
    private JSONObject actualJsonBody;
    private JSONObject expectedJsonBody;
    Logger logger = LogUtils.getLogger(OrderListViewCdskdsTestScript.class);
   
    @BeforeClass
    private void orderListViewCdskdsSetUp() throws customException
    {
        try
        {
            LogUtils.info("Order List View Cdskds SetUp");
            ExtentReport.createTest("Order List View Cdskds SetUp");
            ExtentReport.getTest().log(Status.INFO,"Order List View Cdskds SetUp");

            ActionsMethods.login();
            ActionsMethods.verifyOTP();
            baseURI = EnviromentChanges.getBaseUrl();
            
            Object[][] getUrl = getOrderListViewCdskdsUrl();
            if (getUrl.length > 0) 
            {
                ...
                LogUtils.failure(logger, "No order list view cdskds URL found in test data");
                ExtentReport.getTest().log(Status.FAIL, "No order list view cdskds URL found in test data");
                throw new customException("No order list view cdskds URL found in test data");
            }
            ...
            orderListViewCdskdsRequest = new OrderRequest();
            
        } catch (Exception e) {
            LogUtils.failure(logger, "Error in order list view cdskds setup: " + e.getMessage());
            ExtentReport.getTest().log(Status.FAIL, "Error in order list view cdskds setup: " + e.getMessage());
            throw new customException("Error in order list view cdskds setup: " + e.getMessage());
        }
    }
    
    @DataProvider(name = "getOrderListViewCdskdsUrl")
    private Object[][] getOrderListViewCdskdsUrl() throws customException {
        try {
            LogUtils.info("Reading Order List View Cdskds API endpoint data");
            ExtentReport.getTest().log(Status.INFO, "Reading Order List View Cdskds API endpoint data");
            
            Object[][] readExcelData = DataDriven.readExcelData(excelSheetPathForGetApis, "commonAPI");
            
            if (readExcelData == null || readExcelData.length == 0) {
                String errorMsg = "No Order List View Cdskds API endpoint data found in Excel sheet";
            ...
            Object[][] filteredData = Arrays.stream(readExcelData)
                    .filter(row -> "orderlistviewcdskds".equalsIgnoreCase(row[0].toString()))
                    .toArray(Object[][]::new);
            
            if (filteredData.length == 0) {
                String errorMsg = "No order list view cdskds URL data found after filtering";
            ...
            LogUtils.failure(logger, "Error in getting order list view cdskds URL: " + e.getMessage());
            ExtentReport.getTest().log(Status.FAIL, "Error in getting order list view cdskds URL: " + e.getMessage());
            throw new customException("Error in getting order list view cdskds URL: " + e.getMessage());
        }
    }
    ...
    @DataProvider(name = "getOrderListViewCdskdsNegativeData")
    public Object[][] getOrderListViewCdskdsNegativeData() throws customException {
        try {
            LogUtils.info("Reading order list view cdskds negative test scenario data");
            ExtentReport.getTest().log(Status.INFO, "Reading order list view cdskds negative test scenario data");
            ...
            if (filteredData.isEmpty()) {
                String errorMsg = "No valid order list view cdskds negative test data found after filtering";
                LogUtils.failure(logger, errorMsg);
                ExtentReport.getTest().log(Status.FAIL, MarkupHelper.createLabel(errorMsg, ExtentColor.RED));
                throw new customException(errorMsg);
            }
            ...
            LogUtils.failure(logger, "Error in getting order list view cdskds negative test data: " + e.getMessage());
            ExtentReport.getTest().log(Status.FAIL, "Error in getting order list view cdskds negative test data: " + e.getMessage());
            throw new customException("Error in getting order list view cdskds negative test data: " + e.getMessage());
        }
    }

    @Test(dataProvider = "getOrderListViewCdskdsNegativeData")
    public void orderListViewCdskdsNegativeTest(String apiName, String testCaseid, String testType, String description,
            String httpsmethod, String requestBody, String expectedResponseBody, String statusCode) throws customException {
        try {
            LogUtils.info("Starting order list view cdskds negative test case: " + testCaseid);
            ExtentReport.createTest("Order List View CDSKDS Negative Test - " + testCaseid + ": " + description);
            ExtentReport.getTest().log(Status.INFO, "Test Description: " + description);
            
            if (apiName.equalsIgnoreCase("orderlistviewcdskds") && testType.equalsIgnoreCase("negative")) {
                requestBodyJson = new JSONObject(requestBody);
                
                LogUtils.info("Request Body: " + requestBodyJson.toString());
                ExtentReport.getTest().log(Status.INFO, "Request Body: " + requestBodyJson.toString());
                
                // Set payload for order list view cdskds request
            ...
            String errorMsg = "Error in order list view cdskds negative test: " + e.getMessage();
